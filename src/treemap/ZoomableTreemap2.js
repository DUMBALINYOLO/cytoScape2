...
const ZoomableTreemapData = data;
...
class Text extends Component {
  shouldComponentUpdate({ x, y }) {
    select(this.el).transition().duration(800).attr("x", x).attr("y", y);
    return false;
  }
...
export class ZoomableTreemap extends Component {
  constructor({
    ...
    this.data = data || ZoomableTreemapData;

    this.x = x || scaleLinear();
    this.y = y || scaleLinear();
    this.x.domain([0, width]).range([0, width]);
    this.y.domain([0, height]).range([0, height]);

    const root = hierarchy(this.data)
      .eachBefore((d) => d.values)
      .ancestors((d) => {
  ...
  text({ Comp = Text, d, children, ...rest }) {
    return (
      <Comp x={this.x(d.x0) + 6} y={this.y(d.y0) + 6} {...rest}>
        {children}
      </Comp>
    );
  }
  ...
  key = (d) => {
    return d.parent ? this.key(d.parent) + "." + d.data.key : d.data.key;
  };

  child = (d) => (
    <g
      key={this.key(d)}
      className={d.children && "children"}
      onClick={() => d.children && this.transition(d)}
    >
      {(d.children || [d]).map((d) => this.rect({ d, className: "child" }))}
      {this.rect({
        d,
        className: "parent",
        children: <title>{this.format(d.value)}</title>
      })}
      ...
      <StyledSVG {...svgProps}>
        <g {...gProps}>
          ...
          <g
            ...
            onClick={() => d.parent && this.transition(d.parent)}
          >
            {this.rect({
              d,
              y: -margin.top,
